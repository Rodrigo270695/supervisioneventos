...
import PlanTypeForm from './PlanTypeForm.vue';
...
const currentPlanType = ref<PlanType | null>(null);
...
const handleSearch = (value: string) => {
    ...
    searchForm.get(route('plan-types.index'), {
        preserveState: true,
        preserveScroll: true,
        only: ['planTypes'],
    });
...
const openDeleteModal = (planType: PlanType) => {
    currentPlanType.value = planType;
    showDeleteModal.value = true;
};

// Eliminar tipo de plano
const deletePlanType = () => {
    if (!currentPlanType.value) return;

    deleteForm.delete(route('plan-types.destroy', currentPlanType.value.id), {
        onSuccess: () => {
...
const handlePaginationChange = (page: number) => {
    router.get(route('plan-types.index', { page }), {}, {
        preserveState: true,
        preserveScroll: true,
        only: ['planTypes'],
    });
...
<template>
    <Head title="Gestionar Tipos de Plano" />
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="rounded-xl p-4 sm:p-6">
            <div class="mb-8 flex flex-col justify-between space-y-4 md:flex-row md:items-center md:space-y-0">
                <h1 class="text-2xl font-bold tracking-tight">Gestionar Tipos de Plano</h1>
            ...
            <!-- Vista móvil de tipos de plano -->
            <DataTableMobile
                :columns="columns"
                :items="planTypes.data"
                :search-term="search"
                :hover="false"
                :showChevron="false"
                :pagination="planTypes"
            ...
            <!-- Tabla de tipos de plano usando DataTable (visible solo en desktop) -->
            <DataTable
                :columns="columns"
                :items="planTypes.data"
                :pagination="planTypes"
            ...
            <button
                ...
                aria-label="Crear nuevo tipo de plano"
            >
                <Plus class="h-6 w-6" />
            </button>
        </div>
    ...
    <Modal
        :show="showModal"
        :title="formMode === 'create' ? 'Crear Tipo de Plano' : 'Editar Tipo de Plano'"
        size="md"
    ...
    <ConfirmDeleteModal
        ...
        :item-name="currentPlanType?.name"
        :processing-delete="deleteForm.processing"
        message="¿Estás seguro de que deseas eliminar el tipo de plano?"
        warning-message="Esta acción no se puede deshacer y podría afectar a los planos que utilizan este tipo."
        @close="showDeleteModal = false"
        @confirm="deletePlanType"
    />
</template>
