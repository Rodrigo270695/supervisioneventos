...
const form = useForm({
    id: props.timeType?.id ?? null,
    name: props.timeType?.name ?? '',
    description: props.timeType?.description ?? '',
});

// Determinar la URL y método según el modo
const submitForm = () => {
    if (props.mode === 'create') {
        form.post(route('time-types.store'), {
            onSuccess: () => {
                emit('success', 'Tipo de tiempo creado exitosamente');
                form.reset();
            }
        });
    } else {
        form.put(route('time-types.update', props.timeType?.id), {
            onSuccess: () => {
                emit('success', 'Tipo de tiempo actualizado exitosamente');
            }
        });
    }
};
...
// Watch para actualizar el formulario cuando cambia el modo o el tipo de tiempo
watch(
    [() => props.mode, () => props.timeType],
    () => {
        // Reiniciar el formulario
        form.id = props.timeType?.id ?? null;
        form.name = props.timeType?.name ?? '';
        form.description = props.timeType?.description ?? '';
        form.clearErrors();
    }
);
</script>
